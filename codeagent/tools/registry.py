...
BUILTIN_TOOLS: list[dict[str, Any]] = [
    ...
    {"name": "schedule_cron", "description": "Schedule a cron job. cron is 5-field: min hour dom month dow.", "input_schema": {"type": "object", "properties": {"cron": {"type": "string"}, "prompt": {"type": "string"}, "recurring": {"type": "boolean"}, "durable": {"type": "boolean"}}, "required": ["cron", "prompt"]}},
...
def build_builtin_handlers(runtime: Any) -> dict[str, Callable[..., str]]:
    ...
    return {
        "bash": lambda command, run_in_background=False: run_bash(command, cwd, run_in_background),
        ...
        "create_task": lambda subject, description="", blockedBy=None: _create_task(runtime, subject, description, blockedBy),
        ...
        "schedule_cron": lambda cron, prompt, recurring=True, durable=True: runtime.cron.schedule(cron, prompt, recurring, durable),
        "list_crons": lambda: runtime.cron.list_jobs(),
        "cancel_cron": lambda job_id: runtime.cron.cancel(job_id),
        "spawn_teammate": lambda name, role, prompt: spawn_teammate_thread(runtime, name, role, prompt),
        "send_message": lambda to, content: (runtime.bus.send("lead", to, content), f"Sent to {to}")[1],
        "check_inbox": lambda: _check_inbox(runtime),
        "request_shutdown": lambda teammate: runtime.protocols.request_shutdown(teammate),
        "request_plan": lambda teammate, task: runtime.protocols.request_plan(teammate, task),
        "review_plan": lambda request_id, approve, feedback="": runtime.protocols.review_plan(request_id, approve, feedback),
        "create_worktree": lambda name, task_id="": runtime.worktrees.create(name, task_id),
        "remove_worktree": lambda name, discard_changes=False: runtime.worktrees.remove(name, discard_changes),
        "keep_worktree": lambda name: runtime.worktrees.keep(name),
        "connect_mcp": lambda name: runtime.mcp.connect(name),
    }


def _create_task(runtime: Any, subject: str, description: str = "", blockedBy: list[str] | None = None) -> str:
...
def build_tool_pool(runtime: Any) -> tuple[list[dict[str, Any]], dict[str, Callable[..., str]]]:
